...
// ============================================================================
// 29-AGR - CAP_CUATM_FILIAL DIN JS (v051)
//
...
function agr29TakeOverOneCuatmSelect(selectElement, force) {
    ...
    if (!force && $old.attr('data-agr29-cuatm-owner') === '051' && $old.find('option').length > 1) {
        return $old;
    }
    ...
    $select
        ...
        .attr('data-agr29-cuatm-owner', '051');
    ...
    $select.off('.agr29Cuatm051').on('change.agr29Cuatm051', function () {
        agr29SyncCuatmToValues(jQuery(this));
    });

    agr29SyncCuatmToValues($select);

    console.log('[29-AGR CUATM] v051 OK:', $select.attr('name'), 'options=', $select.find('option').length);
...
function agr29InstallCuatmObserver() {
    ...
    if (!container || !window.MutationObserver || container._agr29CuatmObserver051) {
        return;
    }
    ...
    var observer = new MutationObserver(function (mutations) {
        ...
        if (needRepair) {
            setTimeout(function () {
                jQuery('select[field="CAP_CUATM_FILIAL"]').each(function () {
                    if (this.options.length <= 1 || jQuery(this).attr('data-agr29-cuatm-owner') !== '051') {
    ...
    container._agr29CuatmObserver051 = observer;
}

function agr29BindCuatmAddRow() {
    jQuery(document)
        .off('click.agr29Cuatm051', '.CAP-grid-addrow')
        .on('click.agr29Cuatm051', '.CAP-grid-addrow', function () {
