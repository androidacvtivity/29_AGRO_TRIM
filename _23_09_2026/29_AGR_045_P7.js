(function ($) {
    Drupal.behaviors.agr29 = {
        attach: function (context, settings) {
            ...
            // Delegated events: functioneaza si pentru campurile adaugate dinamic la filiale.
            jQuery(document)
                .off('keypress.agr29Numeric', 'input.numeric')
                .on('keypress.agr29Numeric', 'input.numeric', function (event) {

                    // Campurile numeric trebuie sa accepte doar numere intregi:
                    // blocam explicit virgula si punctul.
                    if (
                        event.key === ',' ||
                        event.key === '.' ||
                        event.which === 44 ||
            ...
            jQuery(document)
                .off('keypress.agr29Float', 'input.float')
                .on('keypress.agr29Float', 'input.float', function (event) {
                    if (isNumberPressed(this, event) === false) {
                        event.preventDefault();
                    }
                });
...
function normalizeDecimalComma() {
    ...
    var selector = 'input.float';

    // Dacă utilizatorul apasă virgula, introducem punct în locul ei.
    jQuery(document)
        .off('keydown.decimalComma', selector)
        .on('keydown.decimalComma', selector, function (event) {
    ...
    // Paste: 12,5 -> 12.5
    jQuery(document)
        .off('input.decimalComma', selector)
        .on('input.decimalComma', selector, function () {
