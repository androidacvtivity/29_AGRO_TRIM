(function ($) {
    Drupal.behaviors.agr29 = {
        attach: function (context, settings) {
            ...
            // CAP_CUATM_FILIAL: lista se incarca direct din JS, nu din clasificatorul CL.
            initAgr29CuatmFromJs();
...
// Select2 este deja initializat de platforma. Dupa ce adaugam optiunile,
// il distrugem si il initializam din nou ca sa citeasca lista noua.
// ============================================================================
...
function agr29GetCuatmIndex($select) {
...
function agr29GetSavedCuatm($select) {
    var value = String($select.val() || '').trim();
    if (value !== '') {
        return value;
    }

    var index = agr29GetCuatmIndex($select);
    var values = Drupal.settings && Drupal.settings.mywebform
        ? Drupal.settings.mywebform.values
        : null;

    if (values && Array.isArray(values.CAP_CUATM_FILIAL) && index !== null) {
        return String(values.CAP_CUATM_FILIAL[index] || '').trim();
    }

    return '';
}

function agr29FillCuatmOptions($select) {
    var savedValue = agr29GetSavedCuatm($select);

    // Dezactivam doar legatura cu vechiul clasificator gol.
...
function agr29RefreshCuatmSelect2($select) {
    // Select2 4.x din platforma pastreaza cache-ul initial (care era gol).
    // De aceea trigger('change') nu este suficient: il reinitalizam o singura data.
    ...
    try {
        ...
        // Daca platforma l-a lasat intr-o stare intermediara, continuam cu init nou.
    }
...
function agr29SyncCuatmValue($select) {
    var index = agr29GetCuatmIndex($select);
    if (index === null || !Drupal.settings || !Drupal.settings.mywebform) {
        return;
    }
...
function initAgr29CuatmFromJs() {
    // Populam toate selecturile, inclusiv sablonul [0].
    jQuery('select[field="CAP_CUATM_FILIAL"]').each(function () {
        var $select = jQuery(this);
        agr29FillCuatmOptions($select);

        // [0] este sablonul ascuns pentru randuri noi; nu pornim Select2 pe el.
        var index = agr29GetCuatmIndex($select);
        if (index !== 0) {
            agr29RefreshCuatmSelect2($select);
        }
    });
}
...
function bindAgr29CuatmFromJs() {
    jQuery(document)
        ...
        .on('change.agr29CuatmFromJs', 'select[field="CAP_CUATM_FILIAL"]', function () {
            agr29SyncCuatmValue(jQuery(this));
        });

    jQuery(document)
        .off('click.agr29CuatmFromJs', '.CAP-grid-addrow')
        .on('click.agr29CuatmFromJs', '.CAP-grid-addrow', function () {
            // Lasam mai intai platforma sa cloneze randul si sa aplice behavior-urile ei.
            setTimeout(function () {
                initAgr29CuatmFromJs();
            }, 100);
        });

    // La prima incarcare, daca Select2-ul platformei se initializeaza dupa agr29,
    // facem inca un refresh scurt dupa ce DOM-ul este stabil.
    setTimeout(function () {
        initAgr29CuatmFromJs();
    }, 150);
}
