(function ($) {
    Drupal.behaviors.agr29 = {
        attach: function (context, settings) {
            ...
            // CAP_CUATM_FILIAL: clasificatorul este gol; lista se populeaza direct din JS.
            agr29InitCuatmFromJs(context);
...
// ============================================================================
// 29-AGR - CAP_CUATM_FILIAL direct din JS
//
// Parametrii originali din Excel pentru CAP_CUATM_FILIAL:
//   grid_name = CAP
//   type      = text
//   widget    = select2
//   class     = dynamic-cuatm
//   maxlength = 15
//   xpath     = dec/DataSet/Data/CAP/CAP_R[line]/CAP_CUATM_FILIAL
//
// Problema: randurile CAP sunt create/inlocuite dinamic. Nu este suficient sa
// populam doar sablonul CAP_CUATM_FILIAL[0]. Trebuie sa populam si fiecare
// select real CAP_CUATM_FILIAL[1], [2], ... exact cand apare in DOM.
// ============================================================================
...
function agr29GetCuatmSavedValue($select) {
    // 1. Daca selectul are deja o valoare (editarea unui raport), o pastram.
    var currentValue = String($select.val() || '').trim();
    if (currentValue !== '') {
        return currentValue;
    }

    // 2. Fallback la Drupal.settings, util cand clasificatorul vechi nu a putut
    //    genera option-ul pentru valoarea deja salvata.
    try {
        var values = Drupal.settings.mywebform.values;
        ...
        var match = name.match(/^CAP_CUATM_FILIAL\[(\d+)\]$/);
        if (!match) {
            return '';
        }
        ...
        // In platforma randul vizibil incepe de la [1], iar [0] este sablonul.
        // Incercam ambele forme fara sa schimbam structura Drupal.settings.
        if (typeof list[rowIndex] !== 'undefined' && list[rowIndex] !== null && String(list[rowIndex]).trim() !== '') {
            return String(list[rowIndex]).trim();
        }

        if (rowIndex > 0 && typeof list[rowIndex - 1] !== 'undefined' && list[rowIndex - 1] !== null) {
            return String(list[rowIndex - 1] || '').trim();
        }
    } catch (e) {
        // Nu blocam formularul daca Drupal.settings nu este disponibil momentan.
    }

    return '';
}

function agr29PopulateOneCuatmSelect(selectElement) {
    var $select = jQuery(selectElement);

    if (!$select.length || $select.attr('field') !== 'CAP_CUATM_FILIAL') {
        return;
    }

    // Daca lista este deja completa, nu o reconstruim la fiecare Drupal attach.
    if ($select.attr('data-agr29-cuatm-ready') === '1' && $select.find('option').length > 1) {
        return;
    }

    var savedValue = agr29GetCuatmSavedValue($select);

    // Oprim doar mecanismul vechi care incearca sa citeasca clasificatorul gol.
    // Clasa select2-widget ramane si Select2-ul platformei ramane functional.
    $select.removeClass('dynamic-cuatm');
    ...
    if (savedValue !== '') {
        // Daca apare o valoare veche care nu mai este in lista curenta, o pastram
        // ca sa nu pierdem date la editarea declaratiei.
        if (!$select.find('option[value="' + savedValue.replace(/"/g, '\\"') + '"]').length) {
            $select.append(jQuery('<option>', {
                value: savedValue,
                text: savedValue
            }));
    ...
    $select.attr('data-agr29-cuatm-ready', '1');

    // IMPORTANT: nu distrugem Select2. Platforma l-a initializat deja.
    // Select2 4.x citeste option-urile noi din select; notificam doar componenta.
    if ($select.hasClass('select2-hidden-accessible')) {
        $select.trigger('change.select2');
    }
}

function agr29PopulateCuatmSelects(context) {
    var $context = context ? jQuery(context) : jQuery(document);
    var $selects = jQuery();

    if ($context.is && $context.is('select[field="CAP_CUATM_FILIAL"]')) {
        $selects = $selects.add($context);
    }

    $selects = $selects.add($context.find('select[field="CAP_CUATM_FILIAL"]'));

    // Daca context este document sau un element care nu contine toate randurile,
    // verificam si global; sunt putine selecturi si costul este neglijabil.
    $selects = $selects.add(jQuery('select[field="CAP_CUATM_FILIAL"]'));

    $selects.each(function () {
        agr29PopulateOneCuatmSelect(this);
    });
}

function agr29ObserveNewCuatmRows() {
    var container = document.querySelector('div#CAP[default-row-count]');

    if (!container || !window.MutationObserver || container._agr29CuatmObserver) {
        return;
    }

    var observer = new MutationObserver(function (mutations) {
        var foundNewCuatmSelect = false;

        for (var i = 0; i < mutations.length && !foundNewCuatmSelect; i++) {
            ...
            for (var j = 0; j < addedNodes.length; j++) {
                ...
                if (node.matches && node.matches('select[field="CAP_CUATM_FILIAL"]')) {
                    foundNewCuatmSelect = true;
                    break;
                }

                if (node.querySelector && node.querySelector('select[field="CAP_CUATM_FILIAL"]')) {
                    foundNewCuatmSelect = true;
                    break;
                }
            }
        ...
        if (foundNewCuatmSelect) {
            // Asteptam ca platforma sa termine indexarea randului si initializarea Select2.
            setTimeout(function () {
                agr29PopulateCuatmSelects(container);
            }, 0);
    ...
    container._agr29CuatmObserver = observer;
}

function agr29BindAddRowFallback() {
    // Handler direct pe buton, nu delegat pe document. Unele handlere ale grilei
    // opresc propagarea click-ului, de aceea varianta delegata poate sa nu ruleze.
    jQuery('.CAP-grid-addrow')
        .off('click.agr29CuatmFromJs')
        .on('click.agr29CuatmFromJs', function () {
            var delays = [0, 50, 150, 300, 600, 1000];

            for (var i = 0; i < delays.length; i++) {
                (function (delay) {
                    setTimeout(function () {
                        agr29PopulateCuatmSelects(document);
                    }, delay);
                })(delays[i]);
            }
        });
...
function agr29InitialCuatmPoll() {
    // La incarcare, grila CAP poate crea primul rand DUPA Drupal.attach().
    // Facem o verificare scurta, apoi oprim timerul automat.
    if (window._agr29CuatmInitialTimer) {
        return;
    }

    var tries = 0;
    window._agr29CuatmInitialTimer = setInterval(function () {
        tries++;
        agr29PopulateCuatmSelects(document);

        var $realRows = jQuery('select[field="CAP_CUATM_FILIAL"]').filter(function () {
            var name = String(jQuery(this).attr('name') || '');
            return !/\[0\]$/.test(name);
        });

        var allReady = $realRows.length > 0;
        ...
        if (allReady || tries >= 40) { // max. ~8 secunde
            clearInterval(window._agr29CuatmInitialTimer);
            window._agr29CuatmInitialTimer = null;
        }
    }, 200);
}

function agr29InitCuatmFromJs(context) {
    agr29PopulateCuatmSelects(context || document);
    agr29ObserveNewCuatmRows();
    agr29BindAddRowFallback();
    agr29InitialCuatmPoll();
}
