(function ($) {
    Drupal.behaviors.agr29 = {
        attach: function (context, settings) {
            ...
            // Populeaza Cod CUATM pentru fiecare raion/filiala din clasificatorul VW_CL_CUATM.xlsx.
            initAgr29CuatmFilialSelectors(context);
...
// ============================================================================
// Cod CUATM - lista raioanelor pentru CAP_CUATM_FILIAL
// Sursa: VW_CL_CUATM.xlsx (CODUL + DENUMIRE)
// Exemplu afisat: 1700000 - R-UL CAHUL
// ============================================================================
...
var AGR29_CUATM_FIELD_SELECTOR = [
...
function agr29GetCuatmFieldIndex($field) {
    ...
    var match = id.match(/^CAP_CUATM_FILIAL-(\d+)$/);
...
function agr29PopulateNativeCuatmSelect($field, savedValue) {
    // Refacem lista pentru a garanta ca select-ul contine exact clasificatorul cerut.
    $field.empty();
    $field.append(jQuery('<option>', { value: '', text: '' }));
    ...
    // Nu pierdem o valoare deja salvata chiar daca, temporar, nu exista in clasificator.
    if (savedValue && !$field.find('option[value="' + savedValue + '"]').length) {
        $field.append(jQuery('<option>', {
            value: savedValue,
    ...
    // Daca platforma nu a initializat Select2 inca, il initializam aici.
...
function agr29PopulateInputCuatmSelect2($field, savedValue) {
    if (typeof $field.select2 !== 'function') {
        return;
    }

    // Pentru configuratia veche a platformei, campul "text + select2" poate fi input.
    // Reinitializam sursa Select2 cu lista din Excel si pastram valoarea existenta.
    try {
        if ($field.data('select2')) {
            $field.select2('destroy');
        }
    } catch (e) {
        // Unele versiuni Select2 nu expun destroy pentru toate tipurile de camp.
    }

    $field.select2({
        data: AGR29_CUATM_OPTIONS,
        placeholder: 'Selectati cod CUATM',
        allowClear: true,
        width: '220px'
    });
    ...
    // Select2 3.x (folosit de platformele vechi) actualizeaza cel mai sigur
    // textul afisat prin comanda 'val'. Pentru Select2 4.x ramanem pe val()+change.
    try {
        if ($field.data('select2')) {
            $field.select2('val', savedValue);
        }
    } catch (e) {
        // Select2 4.x nu mai suporta select2('val', ...).
    }
}

function initAgr29CuatmFilialSelectors(context) {
    var $context = context ? jQuery(context) : jQuery(document);
    var $fields = $context.find(AGR29_CUATM_FIELD_SELECTOR);

    if ($context.is && $context.is(AGR29_CUATM_FIELD_SELECTOR)) {
    ...
    $fields.each(function () {
        ...
        if ($field.is('select')) {
            agr29PopulateNativeCuatmSelect($field, savedValue);
        } else if ($field.is('input')) {
            agr29PopulateInputCuatmSelect2($field, savedValue);
        }
    });
}

function agr29SyncCuatmInternalValue($field) {
    if (
        typeof Drupal === 'undefined' ||
        !Drupal.settings ||
        !Drupal.settings.mywebform ||
    ...
    values.CAP_CUATM_FILIAL[index] = String($field.val() || '').trim();
}
...
function bindAgr29CuatmFilialEvents() {
    ...
    // Evitam dublarea handlerelor, deoarece Drupal.behaviors.attach poate fi apelat repetat.
    $form.off('.agr29Cuatm');

    $form.on(
        'change.agr29Cuatm select2:select.agr29Cuatm select2:unselect.agr29Cuatm',
        AGR29_CUATM_FIELD_SELECTOR,
        function () {
            agr29SyncCuatmInternalValue(jQuery(this));
        }
    );

    // La adaugarea unui raion nou, platforma cloneaza campul CAP_CUATM_FILIAL.
    // Reaplicam lista CUATM dupa ce noul rand este introdus in DOM.
    $form.on('row_added.agr29Cuatm', function () {
        setTimeout(function () {
            initAgr29CuatmFilialSelectors(document);
        }, 0);
    });

    // Compatibilitate cu refresh-ul intern al campurilor din grid.
    $form.on(
        'mywebform:gridRefreshField.agr29Cuatm',
        AGR29_CUATM_FIELD_SELECTOR,
        function () {
            initAgr29CuatmFilialSelectors(this);
        }
    );
}
