(function ($) {
    Drupal.behaviors.agr29 = {
        attach: function (context, settings) {
            ...
            // Cod CUATM din JS pentru @CAP_CUATM_FILIAL.
            initAgr29CuatmFilialFields(context);
            bindAgr29CuatmFilialEvents();
...
// ============================================================================
// Cod CUATM - populare din JS pentru campul @CAP_CUATM_FILIAL
// Afisare: 1700000 - R-UL CAHUL
// Salvare in values.CAP_CUATM_FILIAL: 1700000
// ============================================================================

var AGR29_CUATM_OPTIONS = [
    { id: '0100000', text: '0100000 - MUN.CHISINAU' },
    { id: '0300000', text: '0300000 - MUN.BALTI' },
    { id: '1400000', text: '1400000 - R-UL BRICENI' },
    { id: '3400000', text: '3400000 - R-UL DONDUSENI' },
    { id: '3600000', text: '3600000 - R-UL DROCHIA' },
    { id: '4100000', text: '4100000 - R-UL EDINET' },
    { id: '4300000', text: '4300000 - R-UL FALESTI' },
    { id: '4500000', text: '4500000 - R-UL FLORESTI' },
    { id: '4800000', text: '4800000 - R-UL GLODENI' },
    { id: '6200000', text: '6200000 - R-UL OCNITA' },
    { id: '7100000', text: '7100000 - R-UL RISCANI' },
    { id: '7400000', text: '7400000 - R-UL SINGEREI' },
    { id: '7800000', text: '7800000 - R-UL SOROCA' },
    { id: '1000000', text: '1000000 - R-UL ANENII NOI' },
    { id: '2500000', text: '2500000 - R-UL CALARASI' },
    { id: '3100000', text: '3100000 - R-UL CRIULENI' },
    { id: '3800000', text: '3800000 - R-UL DUBASARI' },
    { id: '5300000', text: '5300000 - R-UL HINCESTI' },
    { id: '5500000', text: '5500000 - R-UL IALOVENI' },
    { id: '6000000', text: '6000000 - R-UL NISPORENI' },
    { id: '6400000', text: '6400000 - R-UL ORHEI' },
    { id: '6700000', text: '6700000 - R-UL REZINA' },
    { id: '8000000', text: '8000000 - R-UL STRASENI' },
    { id: '8300000', text: '8300000 - R-UL SOLDANESTI' },
    { id: '8900000', text: '8900000 - R-UL TELENESTI' },
    { id: '9200000', text: '9200000 - R-UL UNGHENI' },
    { id: '1200000', text: '1200000 - R-UL BASARABEASCA' },
    { id: '1700000', text: '1700000 - R-UL CAHUL' },
    { id: '2100000', text: '2100000 - R-UL CANTEMIR' },
    { id: '2700000', text: '2700000 - R-UL CAUSENI' },
    { id: '2900000', text: '2900000 - R-UL CIMISLIA' },
    { id: '5700000', text: '5700000 - R-UL LEOVA' },
    { id: '8500000', text: '8500000 - R-UL STEFAN VODA' },
    { id: '8700000', text: '8700000 - R-UL TARACLIA' },
    { id: '9600000', text: '9600000 - UTA GAGAUZIA' },
    { id: '9800000', text: '9800000 - UATdS NISTRULUI' }
];

var AGR29_CUATM_ORIGINAL_SELECTOR = [
    'select[field="CAP_CUATM_FILIAL"]',
    'input[field="CAP_CUATM_FILIAL"]',
    'select[name="CAP_CUATM_FILIAL"]',
    'input[name="CAP_CUATM_FILIAL"]',
    'select[name="CAP_CUATM_FILIAL[]"]',
    'input[name="CAP_CUATM_FILIAL[]"]',
    'select[id="CAP_CUATM_FILIAL"]',
    'input[id="CAP_CUATM_FILIAL"]',
    'select[id^="CAP_CUATM_FILIAL-"]',
    'input[id^="CAP_CUATM_FILIAL-"]'
].join(', ');

function agr29GetCuatmOriginalFields(context) {
    var $context = context ? jQuery(context) : jQuery(document);
    var $fields = $context.find(AGR29_CUATM_ORIGINAL_SELECTOR).filter(function () {
        var $el = jQuery(this);
        return !$el.hasClass('agr29-cuatm-proxy') && !$el.closest('.select2-container').length;
    });

    if ($context.is && $context.is(AGR29_CUATM_ORIGINAL_SELECTOR)) {
        $fields = $fields.add($context);
    }
...
function agr29GetCuatmFieldIndex($field) {
    var rowIndex = parseInt($field.attr('row-index'), 10);
    ...
    var idMatch = id.match(/^CAP_CUATM_FILIAL-(\d+)$/);
    if (idMatch) {
    ...
    var $all = agr29GetCuatmOriginalFields(document);
    var index = $all.index($field);
    return index >= 0 ? index : 0;
}

function agr29GetSavedCuatmValue($field) {
    var currentValue = String($field.val() || '').trim();
    if (currentValue !== '') {
        return currentValue;
    }
    ...
    if (
        ...
        Array.isArray(Drupal.settings.mywebform.values.CAP_CUATM_FILIAL)
    ) {
        var index = agr29GetCuatmFieldIndex($field);
        return String(Drupal.settings.mywebform.values.CAP_CUATM_FILIAL[index] || '').trim();
    }

    return '';
}

function agr29CuatmTextById(cuatm) {
    for (var i = 0; i < AGR29_CUATM_OPTIONS.length; i++) {
        if (AGR29_CUATM_OPTIONS[i].id === cuatm) {
            return AGR29_CUATM_OPTIONS[i].text;
        }
    }
    return cuatm;
}

function agr29FillCuatmSelect($select, savedValue) {
    $select.empty();
    ...
    for (var i = 0; i < AGR29_CUATM_OPTIONS.length; i++) {
        $select.append(jQuery('<option>', {
            value: AGR29_CUATM_OPTIONS[i].id,
    ...
    if (savedValue && !$select.find('option[value="' + savedValue + '"]').length) {
        $select.append(jQuery('<option>', {
            value: savedValue,
            text: agr29CuatmTextById(savedValue)
        }));
    }
...
function agr29InitSelect2($select) {
    ...
    $select.select2({
        placeholder: 'Selectati cod CUATM',
        allowClear: true,
        width: '240px'
    });
}

function agr29SyncCuatmValueByOriginalField($originalField, value) {
    var normalized = String(value || '').trim();
    $originalField.val(normalized);
    ...
    if (
        ...
        var values = Drupal.settings.mywebform.values;
        var index = agr29GetCuatmFieldIndex($originalField);

        if (!Array.isArray(values.CAP_CUATM_FILIAL)) {
            values.CAP_CUATM_FILIAL = [];
        }

        values.CAP_CUATM_FILIAL[index] = normalized;
    }
}

function agr29ConfigureOriginalSelect($field, savedValue) {
    agr29FillCuatmSelect($field, savedValue);
    agr29InitSelect2($field);
    agr29SyncCuatmValueByOriginalField($field, savedValue);
    $field.trigger('change');
}

function agr29ConfigureInputWithProxySelect($field, savedValue) {
    try {
        if (typeof $field.select2 === 'function' && $field.data('select2')) {
            $field.select2('destroy');
        }
    } catch (e) {
    }

    var targetId = String($field.attr('id') || 'CAP_CUATM_FILIAL_' + agr29GetCuatmFieldIndex($field));
    var proxySelector = 'select.agr29-cuatm-proxy[data-target-field-id="' + targetId.replace(/"/g, '\\"') + '"]';
    var $proxy = $field.siblings(proxySelector).first();

    if (!$proxy.length) {
        $proxy = jQuery('<select>', {
            'class': 'agr29-cuatm-proxy',
            'data-target-field-id': targetId,
            'aria-label': 'Cod CUATM'
        });
        $field.after($proxy);
    }

    $field.siblings('select.agr29-cuatm-proxy').not($proxy).remove();

    agr29FillCuatmSelect($proxy, savedValue);
    agr29InitSelect2($proxy);

    $field.addClass('agr29-cuatm-original-input').css({
        display: 'none'
    });

    $proxy.off('.agr29CuatmProxy');
    $proxy.on('change.agr29CuatmProxy', function () {
        var value = String(jQuery(this).val() || '').trim();
        agr29SyncCuatmValueByOriginalField($field, value);
        $field.trigger('change');
    });
...
function initAgr29CuatmFilialFields(context) {
    var $fields = agr29GetCuatmOriginalFields(context);

    $fields.each(function () {
        var $field = jQuery(this);
        var savedValue = agr29GetSavedCuatmValue($field);

        if ($field.is('select')) {
            agr29ConfigureOriginalSelect($field, savedValue);
        } else {
            agr29ConfigureInputWithProxySelect($field, savedValue);
        }
    });
}

function bindAgr29CuatmFilialEvents() {
    var $form = jQuery('#mywebform-edit-form');
    if (!$form.length) {
        $form = jQuery(document);
    }

    $form.off('.agr29Cuatm');

    $form.on('change.agr29Cuatm', AGR29_CUATM_ORIGINAL_SELECTOR, function () {
        var $field = jQuery(this);
        agr29SyncCuatmValueByOriginalField($field, $field.val());
    });

    $form.on('change.agr29Cuatm', 'select.agr29-cuatm-proxy', function () {
        var targetId = String(jQuery(this).attr('data-target-field-id') || '');
        var $original = targetId ? jQuery('#' + targetId) : jQuery();
        if ($original.length) {
            agr29SyncCuatmValueByOriginalField($original, jQuery(this).val());
            $original.trigger('change');
        }
    });

    var $cap = jQuery('#CAP');
    if ($cap.length && window.MutationObserver && !$cap.data('agr29CuatmObserverBound')) {
        var observer = new MutationObserver(function (mutations) {
            ...
            if (shouldRefresh) {
                initAgr29CuatmFilialFields($cap.get(0));
            }
        });

        observer.observe($cap.get(0), {
            childList: true,
            subtree: true
        });

        $cap.data('agr29CuatmObserverBound', true);
    }

    $form.on('row_added.agr29Cuatm mywebform:gridRefreshField.agr29Cuatm', function () {
        setTimeout(function () {
            initAgr29CuatmFilialFields(document);
        }, 0);
    });
}
