(function ($) {
    Drupal.behaviors.agr29 = {
        attach: function (context, settings) {
            ...
            // CAP_CUATM_FILIAL: clasificatorul este gol; lista CUATM se incarca direct din JS.
            initAgr29CuatmFromJs(context);
            bindAgr29CuatmFromJs();
...
// ============================================================================
// CAP_CUATM_FILIAL - lista CUATM direct din JS
// IMPORTANT:
// - nu se mai bazeaza pe clasificatorul CL pentru acest camp;
// - se completeaza chiar select-ul generat de @CAP_CUATM_FILIAL;
// - se completeaza si .CAP-drow (sablonul), astfel fiecare raion nou clonat
//   primeste automat aceeasi lista;
// - NU se creeaza al doilea select si NU se foloseste MutationObserver.
// ============================================================================
...
function agr29GetCuatmArrayIndex($select) {
    // Platforma genereaza name="CAP_CUATM_FILIAL[1]", [2] etc.
    // Folosim exact acel index; [0] este sablonul .CAP-drow.
...
function agr29GetCuatmSavedValue($select) {
    ...
    if (
        ...
        var index = agr29GetCuatmArrayIndex($select);
        if (index !== null) {
...
function agr29FillOneCuatmSelect($select) {
    var savedValue = agr29GetCuatmSavedValue($select);

    // Oprim folosirea clasificatorului din CL pentru acest camp.
    // Ramane select2-widget, deci aspectul Select2 al platformei se pastreaza.
    $select.removeClass('dynamic-cuatm');

    // Nu reconstruim de zeci de ori acelasi select.
    if ($select.attr('data-agr29-cuatm-from-js') !== '1') {
        $select.empty();
        ...
        $select.attr('data-agr29-cuatm-from-js', '1');
    }
    ...
    // Daca Select2 este deja initializat, actualizam doar afisarea lui.
    // Nu il distrugem/reinitializam - asta evita blocarea/ingreunarea paginii.
    if ($select.hasClass('select2-hidden-accessible')) {
        $select.trigger('change.select2');
    }
}

function initAgr29CuatmFromJs(context) {
    // Cautam direct campul real generat de @CAP_CUATM_FILIAL.
    // Selectam si randurile vizibile si sablonul ascuns .CAP-drow.
    var $context = context ? jQuery(context) : jQuery(document);
    var selector = 'select[field="CAP_CUATM_FILIAL"]';
    var $selects = $context.find(selector);

    if ($context.is && $context.is(selector)) {
        $selects = $selects.add($context);
    }

    // Drupal poate apela behavior-ul doar pe o portiune a paginii;
    // ne asiguram ca sablonul CAP-drow este populat si el.
    $selects = $selects.add(jQuery('#CAP > .CAP-drow select[field="CAP_CUATM_FILIAL"]'));

    $selects.each(function () {
        agr29FillOneCuatmSelect(jQuery(this));
    });
}

function agr29SyncCuatmToDrupalValues($select) {
    if (
        typeof Drupal === 'undefined' ||
        !Drupal.settings ||
        !Drupal.settings.mywebform ||
    ...
    values.CAP_CUATM_FILIAL[index] = String($select.val() || '').trim();
}

function bindAgr29CuatmFromJs() {
    // Evitam handler-ele duplicate la apelurile repetate Drupal.attachBehaviors().
    jQuery(document)
        .off('change.agr29CuatmFromJs', 'select[field="CAP_CUATM_FILIAL"]')
        .on('change.agr29CuatmFromJs', 'select[field="CAP_CUATM_FILIAL"]', function () {
            agr29SyncCuatmToDrupalValues(jQuery(this));
        });

    // Daca modulul de grid cloneaza un sablon vechi/cached, dupa click mai facem
    // o populare simpla. Fara observer si fara bucla continua.
    jQuery(document)
        .off('click.agr29CuatmFromJs', '.CAP-grid-addrow')
        .on('click.agr29CuatmFromJs', '.CAP-grid-addrow', function () {
            setTimeout(function () {
                initAgr29CuatmFromJs(document);
            }, 0);
        });
}
